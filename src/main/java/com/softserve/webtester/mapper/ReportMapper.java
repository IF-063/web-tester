...
public interface ReportMapper {
    ...
    @Select({ "<script>SELECT serviceId, buildVersionId, AVG(responseTime) as responseTime FROM ResultHistory",
            ...
            " #{item} </foreach>", "GROUP BY serviceId, buildVersionId </script>" })
    @Results({
            @Result(property = "serviceName", column = "serviceId", one = @One(select = "com.softserve.webtester.mapper.ServiceMapper.loadServiceName") ),
            @Result(property = "responseTime", column = "responseTime", jdbcType = JdbcType.INTEGER),
            @Result(property = "buildVersionName", column = "buildVersionId", one = @One(select = "com.softserve.webtester.mapper.BuildVersionMapper.loadBuildVersionName") ) })
