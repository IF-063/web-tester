			...
			<div class="col-md-12">
				<div class="panel panel-default">
					<div class="panel-heading">
						<form:form modelAttribute="requestCollectionFilterDTO"
							method="GET">
							<fieldset>
								<!--  <legend>Filters</legend> -->

								<div class="col-md-2">
									<label for="requestCollectionNameFilter" class="control-label">Collection
										name</label>
									<form:input type="text" path="requestCollectionNameFilter"
										class="form-control" placeholder="name starts with" />
								</div>

								<div class="col-md-2">
									<label for="labelFilter" class="control-label">Labels</label>
									<form:select path="labelFilter" items="${labels}"
						...
						<div class="row">
							<div class="col-md-12">
								...
								<button id="runSelected" class="btn btn-info">Run
									selected</button>
					...
					<div class="table-responsive">
						<table
							class="table table-hover table-bordered table-condensed text-center panel-body"
							id="requestCollectionsTable">
							<thead>
								<tr>
							...
							<tbody>
								<c:forEach items="${collectionList}" var="requestCollection">
									<tr id="requestRow">
										...
										<td><c:out value="${requestCollection.description}"></c:out>
										</td>
										<td><c:forEach items="${requestCollection.labels}"
												var="label">${label.name}, </c:forEach></td>
										<td><a
											href=<c:url value="/results/requestCollections/${requestCollection.id}" />>results</a></td>
										<td><a
											href=<c:url value="/tests/collections/newCollection?fromId=${requestCollection.id}" />>
												<i class="fa fa-copy fa-lg"></i>
										</a></td>
										<td><input id=<c:out value="${requestCollection.id}" />
											type="checkbox" name="disableSelect"></td>
										<td><a id="${requestCollection.id}"
											class="removeInstance"><i class="fa fa-trash fa-lg"></i></a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
